  ...
  return (
    <nav className="fixed bottom-0 z-50 w-full sm:sticky sm:w-auto sm:self-start">
      ...
      <ul className="flex cursor-pointer gap-1 border-t sm:flex-col sm:border-none">
        <NavigationItem label="さがす" icon={<AiOutlineSearch className="text-2xl" />} />
        <NavigationItem label="お気に入り" icon={<AiOutlineHeart className="text-2xl" />} />
        <NavigationItem label="お買い物" icon={<AiOutlineShoppingCart className="text-2xl" />} />
      </ul>
    </nav>
  );
};
